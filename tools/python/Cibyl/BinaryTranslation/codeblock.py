...
class CodeBlock:
    ...
    def compile(self):
        ...
        for insn in self.instructions:
            ...
            if insn.delayed:
                ...
                if insn.delayed.address in self.labels:
    ...
    def splitByAddresses(self, start, end):
        ...
        return [x for x in self.instructions if x.address >= start and x.address < end], dict([(k,v) for k,v in self.labels.items() if v.address >= start and v.address < end])
